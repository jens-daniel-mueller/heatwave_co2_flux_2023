...
region_masks_all <-
  read_ncdf(paste(path_reccap2, "supplementary/RECCAP2_region_masks_all_v20221025.nc", sep = "")) %>%
  as_tibble()
...
region_masks_all <- region_masks_all %>%
  mutate(biome = paste(name, value, sep = "_")) %>% 
...
library(ncdf4)
nc <- nc_open(paste0(path_pCO2_products,"VLIZ-SOM_FFN/VLIZ-SOM_FFN_vBAMS2024.nc"))
print(nc)
...
SOM_FFN <- read_ncdf(paste0(path_pCO2_products,"VLIZ-SOM_FFN/VLIZ-SOM_FFN_vBAMS2024.nc"),
                     make_units = FALSE)

SOM_FFN_tibble <- SOM_FFN %>% 
  as_tibble()
...
SOM_FFN_tibble <-
  inner_join(SOM_FFN_tibble,
             region_masks_all)

SOM_FFN_tibble %>%
  filter(time == max(time)) %>%
  ggplot(aes(lon, lat, fill = fgco2_smoothed)) +
  geom_raster() +
...
SOM_FFN_2023_anomaly %>%
  filter(name == "sol") %>% 
  ggplot(aes(lon, lat, fill = anomaly)) +
  geom_raster() +
  scale_fill_divergent() +
  coord_quickmap(expand = 0)
...
SOM_FFN_monthly_global <-
  ...
  summarise(across(dco2:fgco2_smoothed, 
                   ...
                   .names = "{.col}_mean"),
            across(fgco2_raw:fgco2_smoothed, 
...
SOM_FFN_monthly <- 
  bind_rows(SOM_FFN_monthly_global %>% mutate(biome = "global"),
            SOM_FFN_monthly_biome)



SOM_FFN_monthly %>%
  pivot_longer(-c(time, biome)) %>%
...
SOM_FFN_monthly %>%
  ...
  filter(biome %in% c("global", "NA-SPSS", "NA-STPS", "NP-SPSS"),
         name %in% c("dco2_mean", "atm_co2_mean", "sol_mean", "kw_mean", 
                     "spco2_smoothed_mean", "fgco2_smoothed_integral")) %>%
...
```{r regional_seasonality, fig.asp=1.5}
