...
name_divergent <- c("dco2", "fgco2_smoothed")
...
SOM_FFN <-
  SOM_FFN %>%
  mutate(across(dco2:fgco2_smoothed, ~ if_else(is.na(biome), NA, .)))

# map +
#   geom_tile(data = SOM_FFN %>% filter(time == max(time),
#                                       !is.na(fgco2_smoothed)),
#             aes(lon, lat))
...
The following maps show the absolute state of each variable in 2023 as provided through the pCO2 product, the change in that variable from 1990 to 2023, as well es the anomalies in 2023 from the prediction of a linear/quadratic fit to the data from 1990 to 2022.
...
```{r 2023_monthly_mean_maps, fig.asp=2}
...
```{r change_1990_2023_monthly_mean_maps, fig.asp=2}
...
```{r anomaly_2023_monthly_mean_maps, fig.asp=2}
...
```{r hovmoeller_annual, fig.asp=0.5}
...
```{r hovmoeller_monthly, fig.asp=0.5}

SOM_FFN_hovmoeller_monthly <-
  SOM_FFN %>%
  group_by(year, month, lat) %>%
  summarise(
    across(dco2:spco2_smoothed,
           ~ weighted.mean(., area, na.rm = TRUE)),
    across(fgco2_smoothed,
           ~ sum(. * area, na.rm = TRUE) * 12.01 * 1e-15)
  ) %>%
...
## Monthly means since 2021

```{r hovmoeller_monthly_recent_years}
