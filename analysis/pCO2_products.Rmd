...
```{r anomaly_detection_function, eval=FALSE}
...
```{r anomaly_detection_function_nested_parallel, eval=FALSE}
...
anomaly_determination_parallel <- function(df,...) {
  ...
  df_lm <-
    ...
    nest(data = -c(name, !!!group_by)) %>%
    mutate(
      fit = future_map(data, ~ lm(value ~ year, data = .x),
  ...
  df_quadratic <-
    ...
    nest(data = -c(name, !!!group_by)) %>%
    mutate(
      fit = future_map(data, ~ lm(value ~ year + I(year ^ 2), data = .x),
                .options = furrr_options(packages = "broom")),
      tidied = map(fit, tidy),
      augmented = map(fit, augment)
    )
