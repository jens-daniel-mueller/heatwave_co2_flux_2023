...
nc <-
  nc_open(paste0(
    path_pCO2_products,
    "GCB-2024_dataprod_UExP-FNN-U-v3_1985-2023.nc"
  ))
...
# pco2_product %>%
#   distinct(time) %>%
#   mutate(time = ymd("1985-01-15") %m+% months(time+0.5)) %>%
#   pull
...
pco2_product <-
  pco2_product %>%
  filter(if_any(
    c("fco2atm", "fgco2", "kw", "alpha", "sfco2", "tos"),
    ~ !is.na(.x)
  ))
...
# pco2_product %>% 
#   filter(if_any(c("fco2atm", "fgco2", "kw", "alpha", "sfco2", "tos"), ~ !is.na(.x))) %>%
#   distinct(time) %>% 
#   print(n=Inf)
