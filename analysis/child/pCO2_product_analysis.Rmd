...
```{r color_definition, include=FALSE}
...
```{r fCO2_decomposition_function_residual_approach}
...
```{r fCO2_decomposition_function_direct_approach, eval=FALSE}
...
```{r compute_global_means_and_integrals_predicted_fluxes}
...
# pco2_product_biome_monthly_fCO2_decomposition_no_weight <-
#   pco2_product_biome_monthly_fCO2_decomposition %>%
...
#   drop_na()
# 
# pco2_product_biome_monthly_fCO2_decomposition_area <-
#   pco2_product_biome_monthly_fCO2_decomposition %>%
#   group_by(year, month, biome, name) %>%
#   summarise(resid = weighted.mean(resid, area, na.rm = TRUE)) %>%
#   ungroup() %>%
#   drop_na()


# compute regional averages weighted by area and wind speed (represented by kw*sol)
...
pco2_product_biome_monthly_fCO2_decomposition <-
  pco2_product_biome_monthly_fCO2_decomposition %>%
  group_by(year, month, biome, name) %>%
  summarise(resid = weighted.mean(resid, area*kw_sol, na.rm = TRUE)) %>%
  ungroup() %>%
  drop_na()
...
```{r {{year_anom}}_anomaly_zonal_mean_sections_annual, fig.asp=1, eval=FALSE, results = FALSE}
...
```{r write_anomaly_anomaly_year_zonal_mean_sections_annual, eval=FALSE}
