...
# 
# map +
#   geom_tile(data = region_masks_all %>% filter(!is.na(region)),
#             aes(lon, lat, fill = region)) +
#   labs(title = "Selected large regions") +
#   scale_fill_muted() +
#   theme(legend.title = element_blank())
...
```{r write_anomaly_2023_annual_mean_maps}

pco2_product_coarse_annual_regression %>%
  filter(year == 2023) %>%
  write_csv(paste0("../data/","{{product_name}}","_anomaly_map_annual.csv"))
...
```{r write_anomaly_2023_monthly_mean_maps}

pco2_product_coarse_monthly_regression %>%
  filter(year == 2023) %>%
  write_csv(paste0("../data/","{{product_name}}","_anomaly_map_monthly.csv"))
...
pco2_product_hovmoeller_monthly_regression %>%
  write_csv(paste0("../data/","{{product_name}}","_anomaly_hovmoeller_monthly.csv"))
...
rm(pco2_product_monthly_global,
   pco2_product_monthly_biome)
...
```{r absolute_biome_means_and_integrals_overview, fig.asp=fig.height}
...
pco2_product_annual_regression %>%
  write_csv(paste0("../data/","{{product_name}}","_biome_annual_regression.csv"))
...
pco2_product_annual_detrended %>%
  write_csv(paste0("../data/","{{product_name}}","_biome_annual_detrended.csv"))
...
pco2_product_annual_regression %>%
  ...
  geom_point(data = . %>% filter(year <= 2022),
             ...
             shape = 21) +
  geom_smooth(data = . %>% filter(year <= 2022),
              method = "lm", se = FALSE, aes(col = "Regression fit\n prior 2023")) +
  scale_color_grey() +
  scale_fill_grayC() +
  new_scale_fill() +
  geom_point(data = . %>% filter(year >= 2022),
...
```{r anomalies_correlation_realtive_to_spread_monthly, fig.asp=1.8}
