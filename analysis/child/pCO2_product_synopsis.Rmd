...
pco2_product_flux_attribution <-
pco2_product_flux_attribution %>% 
  mutate(fgco2_dfco2 = resid_dfco2 * kw * sol * area * 12.01 * 1e-15,
         fgco2_kw_sol = resid_kw * resid_sol * dfco2 * area * 12.01 * 1e-15,
         fgco2_sum = fgco2_dfco2 + fgco2_kw_sol) %>% 
  select(-area)
...
pco2_product_flux_attribution <-
  inner_join(
    pco2_product_flux_attribution,
    pco2_product_annual_detrended %>%
      filter(name %in% c("fgco2_int")) %>%
      select(-c(time, value, fit, name)) %>%
      rename(fgco2_true = resid)
  )


pco2_product_flux_attribution <-
  pco2_product_flux_attribution %>%
  mutate(fgco2_resid = fgco2_true - fgco2_sum)

pco2_product_flux_attribution <-
  pco2_product_flux_attribution %>%
  select(-c("resid_kw", "resid_dfco2", "resid_sol", "kw", "dfco2", "sol")) %>%
