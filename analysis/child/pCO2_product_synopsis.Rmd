...
```{r order_variable_names, eval=FALSE}
...
pco2_product_biome_monthly_detrended <- pco2_product_biome_monthly_detrended %>%
  mutate(
    name = factor(name, levels = name_core),
    product = factor(product, levels = all_product_list)
  ) %>%
...
```{r {{year_anom}}_anomaly_zonal_mean_sections_annual, eval=FALSE}
