...
labels_breaks <- function(i_name) {
  ...
  if (i_name == "intpp") {
    i_legend_title <- "NPP<sub>int</sub> anom.<br>(mol s<sup>-1</sup> m<sup>-2</sup>)"
    i_breaks <- c(-Inf, seq(-1e-7, 1e-7, 2.5e-8), Inf)
  }
  ...
  if (i_name == "dissic") {
    ...
    i_breaks <- c(-Inf, seq(-25, 25, 5), Inf)
  }
  ...
  if (i_name == "mld") {
    ...
    i_breaks <- c(-Inf, seq(-100, 100, 25), Inf)
  }
  ...
  if (i_name == "resid_fgco2_dfco2_kw_sol") {
    i_legend_title <-
      "k<sub>w</sub> K<sub>0</sub> X ΔfCO<sub>2</sub> contr."
    i_breaks <- c(-Inf, seq(-0.5, 0.5, 0.1), Inf)
  }
...
bbox <- st_bbox(c(xmin = -180, xmax = 180, ymax = 80, ymin = -80), crs = st_crs(4326))
bbox <- st_as_sfc(bbox)
...
p_map_mdim_robinson <-
  function(df,
    ...
    if (is.null(dim_col) & is.null(dim_row) & is.null(dim_wrap)) {
      ...
      if (!is.null(dim_col) & !is.null(dim_row)) {
        names_sep <- "_"
      } else {
        names_sep <- NULL
      }
    ...
    if (!(is.null(dim_row) & is.null(dim_col))) {
      ...
      p_map <- p_map +
        facet_grid(as.formula(paste(dim_row, "~", dim_col)), switch = "y")
...
#                 col = "grey80", size = 1) +
#       labs(title =  paste({{year_anom}}, "fCO2 product ensemble mean anomaly")) +
#       scale_fill_gradientn(
#         colours = warm_cool_gradient,
#         rescaler = ~ scales::rescale_mid(.x, mid = 0),
#         name = labels_breaks(.x %>% distinct(name))$i_legend_title,
...
#                 aes(lon, lat, shape = "Ensemble mean\n< StDev"),
#                 col = "grey80", size = 1) +
#       labs(title =  paste({{year_anom}}, "fCO2 product ensemble mean absolute")) +
#       scale_fill_gradientn(
#         colours = warm_cool_gradient,
#         rescaler = ~ scales::rescale_mid(.x, mid = 0),
#         name = labels_breaks(.x %>% distinct(name))$i_legend_title,
...
pco2_product_map_monthly_anomaly_ensemble %>%
  ...
  map(
    ~ map +
      geom_tile(data = .x,
                aes(lon, lat, fill = resid_mean)) +
      labs(title =  paste({{year_anom}}, "fCO2 product ensemble mean monthly anomaly")) +
      scale_fill_gradientn(
        colours = warm_cool_gradient,
        rescaler = ~ scales::rescale_mid(.x, mid = 0),
        name = labels_breaks(.x %>% distinct(name))$i_legend_title,
...
map +
  ...
  scale_fill_gradientn(
    colours = warm_cool_gradient,
    rescaler = ~ scales::rescale_mid(.x, mid = 0),
    name = labels_breaks("sfco2")$i_legend_title,
...
pco2_product_map_annual_flux_attribution_ensemble %>%
  ...
  p_map_mdim_robinson(
    ...
    breaks = 2*(labels_breaks("fgco2")$i_breaks),
    dim_wrap = "name",
    n_col = 1,
    n_labels = 2
  )
...
ggsave(width = 9,
       height = 3,
       dpi = 600,
       filename = "../output/sbiome_seasonal_anomaly_fgco2_attribution_all_products.jpg")
