...
```{r read_all_files, results = FALSE}
...
# SOM_FFN_predict_2024_map_annual_anomaly <-
#   read_csv(
#     "/home/jenmueller/Projects/heatwave_co2_flux_2023/data/SOM-FFN_predict_2024_map_annual_anomaly.csv"
#   )

SOM_FFN_predict_2024_map_annual_anomaly_temperature_predict <-
  read_csv(
    "/home/jenmueller/Projects/heatwave_co2_flux_2023/data/SOM-FFN_predict_2024_map_annual_anomaly_temperature_predict.csv"
  )

SOM_FFN_predict_2024_biome_annual_anomaly <-
  read_csv(
    "/home/jenmueller/Projects/heatwave_co2_flux_2023/data/SOM-FFN_predict_2024_biome_annual_anomaly.csv"
  )
...
The following maps show the anomalies of each variable in `r {{year_anom}}` as provided through the fCO2 product. Anomalies are determined based on the predicted value of a linear regression model fit to the available data from 1990 to `r {{year_anom}}-1`.
...
```{r anomaly_annual_mean_maps_ensemble_mean_offset, fig.asp=1.5, eval=FALSE}
...
ggsave(width = 2.5,
       height = 4,
       filename = "../output/zonal_mean_anomaly_pco2_product_ensemble_mean_warm_cold_color.jpg")
...
```{r fgco2_anomaly_prediction_map}
...
```{r fgco2_anomaly_prediction_map_2024}



SOM_FFN_predict_map_annual_anomaly_temperature_predict <-
  bind_rows(
    SOM_FFN_predict_2024_map_annual_anomaly_temperature_predict %>%
      pivot_longer(-c(year, lon, lat), values_to = "resid"),
    pco2_product_map_annual_anomaly_temperature_predict %>%
      filter(product == "SOM-FFN") %>%
      select(-product) %>%
      mutate(year = 2023)
  )
...
ggsave(width = 9,
       height = 6,
       dpi = 600,
       filename = "../output/map_anomaly_correlation_SOM-FFN.jpg")


SOM_FFN_predict_map_annual_anomaly_temperature_predict %>%
  filter(str_detect(name, "temperature")) %>%
  p_map_mdim_robinson(
    var = "resid",
    legend_title = labels_breaks("temperature")$i_legend_title,
    breaks = labels_breaks("temperature")$i_breaks,
    dim_row = "year"
  )
...
ggsave(width = 9,
       height = 6,
       dpi = 600,
       filename = "../output/map_anomaly_sst.jpg")
...
ggsave(width = 15,
       height = 6,
       dpi = 600,
       filename = "../output/map_flux_anomaly_SOM-FFN.jpg")
...
```{r anomaly_2023_monthly_mean_maps_biome_mask}
...
```{r anomaly_2023_monthly_mean_maps, fig.asp=1.5, eval=FALSE}
...
```{r fCO2_decomposition_monthly_maps_biome_mask}
...
```{r fCO2_decomposition_monthly_maps, fig.asp=1.8, eval=FALSE}
...
```{r flux_attribution_seasonal_maps_biome_mask}
...
```{r flux_attribution_seasonal_maps, fig.asp=1.8, eval=FALSE}
...
pco2_product_hovmoeller_annual_anomaly_ensemble <- 
pco2_product_hovmoeller_monthly_anomaly_ensemble %>% 
  mutate(year = floor(decimal)) %>% 
  group_by(year, lat, name) %>% 
  summarise(resid_mean = mean(resid_mean),
            resid_sd = mean(resid_sd)) %>% 
  ungroup()
  

p_hov_sst <- pco2_product_hovmoeller_annual_anomaly_ensemble %>%
  filter(name == "temperature") %>% 
  ggplot(aes(resid_mean, lat, group = year)) +
  geom_vline(xintercept = 0) +
  geom_path(data = . %>% filter(!year %in% c({{year_anom}}, 1997, 2015)),
            aes(col = "Other years\n1990-2022"),
            alpha = 0.5) +
  ...
  new_scale_fill() +
  new_scale_color() +
  geom_path(data = . %>% filter(year %in% 2002:2007),
            aes(col = "2002-2007"),
            alpha = 0.5) +
  scale_color_manual(values = cold_color, name = "X") +
  scale_fill_manual(values = cold_color, name = "X") +
  new_scale_fill() +
  new_scale_color() +
  geom_path(data = . %>% filter(year %in% c({{year_anom}}, 1997, 2015)),
            aes(col = as.factor(year)),
  ...
  scale_color_manual(values = rev(warm_cool_gradient[c(17, 13, 20)]),
                     guide = guide_legend(reverse = TRUE, order = 2)) +
  labs(x = labels_breaks("temperature")$i_legend_title,
       y = "Latitude (°N)") +
  scale_y_continuous(breaks = seq(-60, 60, 30), expand = c(0,0)) +
  theme_bw() +
  theme(axis.title = element_markdown(),
        legend.title = element_blank(),
        panel.grid.minor = element_blank())

p_hov_fgco2 <- pco2_product_hovmoeller_annual_anomaly_ensemble %>%
  filter(name == "fgco2_hov") %>% 
  ggplot(aes(resid_mean, lat, group = year)) +
  geom_vline(xintercept = 0) +
  geom_path(data = . %>% filter(!year %in% c({{year_anom}}, 1997, 2015)),
            aes(col = "Other years\n1990-2022"),
  ...
  scale_color_manual(values = rev(warm_cool_gradient[c(17, 13, 20)]),
                     guide = guide_legend(reverse = TRUE, order = 2)) +
  labs(x = labels_breaks("fgco2_hov")$i_legend_title,
       y = "Latitude (°N)") +
  scale_y_continuous(breaks = seq(-60, 60, 30), expand = c(0,0)) +
  theme_bw() +
  theme(axis.title = element_markdown(),
...
p_hov_sst + p_hov_fgco2  +
   plot_layout(guides = 'collect')

ggsave(width = 7,
       height = 7,
       dpi = 600,
       filename = "../output/zonal_mean_anomaly_sst_co2flux_ensemble_mean_pco2_products.jpg")

```

```{r hovmoeller_monthly_anomalies_ensemble_mean_offset, fig.asp=0.8, eval=FALSE}
...
The following plots show biome-, super biome- or global- averaged/integrated values of each variable as provided through the fCO2 product, represented here as the anomalies from the prediction of a linear/quadratic fit to the data from 1990 to `r {{year_anom}}-1`.
...
pco2_product_biome_annual_anomaly %>%
  ...
  ggplot(aes(year, resid, col = product)) +
  geom_hline(yintercept = 0) +
  geom_path() +
...
lm_fgco2_sst %>%
  select(product, year, fgco2_int, resid) %>% 
  pivot_longer(c(fgco2_int, resid)) %>% 
  ggplot(aes(year, value, col = product)) +
  geom_hline(yintercept = 0) +
  geom_path() +
  facet_wrap(~name) +
  labs(title = "Unexpected residuals in ocean carbon sink",
       subtitle = "Linear regression baseline + global mean SST anomaly")
...
```{r absolute_integrals_trends_2024, fig.asp=1}

SOM_FFN_biome_annual_anomaly <-
  bind_rows(
    pco2_product_biome_annual_anomaly %>%
      filter(
        name %in% c("fgco2_int", "temperature"),
      ...
      select(-product) %>%
      mutate(year_anom = "2023"),
    SOM_FFN_predict_2024_biome_annual_anomaly %>%
      filter(
        name %in% c("fgco2_int", "temperature"),
        biome == "Global non-polar"
      ) %>%
      mutate(year_anom = "2024")
  )

p_value <- SOM_FFN_biome_annual_anomaly %>% 
  ggplot(aes(year, value, col = year_anom)) +
  geom_path() +
  facet_wrap(
    . ~ name,
    scales = "free_y",
...
p_resid <- SOM_FFN_biome_annual_anomaly %>% 
  ggplot(aes(year, resid, col = year_anom)) +
...
ggsave(width = 8,
       height = 4,
       dpi = 600,
       filename = "../output/timeseries_flux_sst_SOM-FFN.jpg")

SOM_FFN_biome_annual_anomaly %>% 
  select(name, resid, year_anom, year) %>% 
  pivot_wider(values_from = resid) %>% 
  ggplot(aes(temperature, fgco2_int)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~ year_anom)


lm_fgco2_sst <- SOM_FFN_biome_annual_anomaly %>%
  select(year, year_anom, name, resid) %>%
  pivot_wider(values_from = resid) %>%
  nest(data = -year_anom) %>%
  mutate(fit = map(data, ~ lm(
    formula = fgco2_int ~ temperature,
...
lm_fgco2_sst %>% 
  select(year, year_anom, fgco2_int, resid) %>% 
  pivot_longer(c(fgco2_int, resid)) %>% 
  ggplot(aes(year, value, col = year_anom)) +
  geom_hline(yintercept = 0) +
  geom_path() +
...
lm_fgco2_sst %>% 
  ggplot(aes(year, fgco2_int-resid, col = year_anom)) +
  geom_hline(yintercept = 0) +
  geom_path() +
  scale_color_okabeito() +
  labs(title = "Unexpected residuals in ocean carbon sink",
       subtitle = "Linear regression baseline + global mean SST anomaly",
       y = paste("Unexpected", labels_breaks("fgco2_int")$i_legend_title)) +
  theme(legend.title = element_blank(),
        axis.title.x = element_blank(),
        axis.title.y = element_markdown())
...
ggsave(width = 8,
       height = 4,
       dpi = 600,
       filename = "../output/timeseries_flux_anomaly_SOM-FFN.jpg")

# SOM_FFN_biome_annual_anomaly_lm_fgco2_sst <-
#   inner_join(
#     SOM_FFN_biome_annual_anomaly %>%
#       filter(
#         name %in% c("fgco2_int")
#       ) %>%
#       select(year, year_anom, baseline = fit),
#     lm_fgco2_sst %>%
#       select(year, year_anom, resid)
#   )
# 
# 
# lm_fgco2_sst <-
#   lm_fgco2_sst %>%
#   mutate(unexplained = fgco2_int - resid)
# 
# 
# lm_fgco2_sst %>% 
#   ggplot(aes(year, unexplained, col = year_anom)) +
#   geom_hline(yintercept = 0) +
#   geom_path() +
#   labs(title = "Unexpected residuals in ocean carbon sink",
#        subtitle = "Linear regression baseline + global mean SST anomaly")
...
```{r baseline_definitions_testing}
...
mlr_ensemble %>% 
  mutate(name = case_when(
    name == "atm_nino" ~ "Multiple linear regression",
    name == "baseline_global_SST" ~ "Linear baseline + global mean SST",
  )) %>% 
  ggplot(aes(year, value,
             ymin = value - sd,
             ymax = value + sd)) +
  ...
  scale_color_okabeito() +
  facet_wrap(~ name) +
  labs(y = paste("Unexpected", labels_breaks("fgco2_int")$i_legend_title)) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_markdown(),
        legend.title = element_blank())
...
```{r absolute_anomaly_integrals_merged_table}

pco2_product_biome_annual_anomaly_super_regions <-
  bind_rows(
    pco2_product_biome_annual_anomaly_super_regions %>%
      filter(region != "Global ice-free") %>% 
      rename(biome = region),
    pco2_product_biome_annual_anomaly %>% 
      filter(biome != "Global non-polar")
  ) %>%
...
pco2_product_biome_annual_anomaly_super_regions %>%
  ...
  mutate(
    resid = paste(ifelse(
      resid > 0, paste0("+", format(round(resid, 2), nsmall = 2)), format(round(resid, 2), nsmall = 2)
    ), round(resid_sd, 2), sep = "±"),
    value = paste(ifelse(
      value > 0, paste0("+", format(round(value, 2), nsmall = 2)), format(round(value, 2), nsmall = 2)
...
```{r anomalies_from_annual_mean_trends_overview_OceanSODAv2, fig.asp=1}
...
ggsave(width = 5,
       height = 2,
       dpi = 600,
       filename = "../output/global_seasonality_fgco2_ensemble_mean_pco2_products.jpg")
...
ggsave(width = 10,
       height = 4,
       dpi = 600,
       filename = "../output/global_seasonality_sst_all_products.jpg")
...
pco2_product_biome_annual_detrended %>%
  ...
  p_season(title = "Anomalies from predicted annual mean | Global non-polar")
...
pco2_product_biome_annual_anomaly_super_regions %>%
  ...
  geom_point(data = . %>% filter(!year %in% c({{year_anom}}, 1997, 2015)),
             aes(fill = "1990-2022"),
             shape = 21) +
  scale_color_manual(values = "grey60", name = "X") +
  scale_fill_manual(values = "grey60", name = "X") +
...
```{r fgco2_anomaly_prediction_biome}
...
```{r anomalies_correlation_absolute_monthly_global, fig.asp=0.5}
...
```{r anomalies_correlation_absolute_monthly_biomes_key, fig.asp=1}
...
pco2_product_biome_annual_fCO2_decomposition %>%
  filter(year == {{year_anom}}, biome %in% c("PEQU-E", "NA-STPS")) %>%
  mutate(name = case_when(
...
pco2_product_profiles_monthly_conversion %>% 
  ...
  geom_hline(yintercept = 0) +
  geom_path() +
  facet_grid(biome ~ name, scales = "free_y")
