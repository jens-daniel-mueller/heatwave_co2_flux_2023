...
labels_breaks <- function(i_name) {
  ...
  if (i_name == "sfco2_total") {
    i_legend_title <- "fCO<sub>2,ocean</sub> anom.<br>total<br>(µatm)"
    i_breaks <- c(-Inf, seq(-0.5, 0.5, 0.1), Inf)
  }
  
  if (i_name == "sfco2_therm") {
    i_legend_title <- "fCO<sub>2,ocean</sub> anom.<br>thermal<br>(µatm)"
    i_breaks <- c(-Inf, seq(-0.5, 0.5, 0.1), Inf)
  }
  
  if (i_name == "sfco2_nontherm") {
    i_legend_title <- "fCO<sub>2,ocean</sub> anom.<br>non-thermal<br>(µatm)"
    i_breaks <- c(-Inf, seq(-0.5, 0.5, 0.1), Inf)
  }
  ...
  if (i_name == "resid_fgco2_dfco2") {
    i_legend_title <-
      "FCO<sub>2</sub> anom.<br>ΔfCO<sub>2</sub> contr.<br>(mol m<sup>-2</sup> yr<sup>-1</sup>)"
    i_breaks <- c(-Inf, seq(-0.5, 0.5, 0.1), Inf)
  }
  
  if (i_name == "resid_fgco2_kw_sol") {
    i_legend_title <-
      "FCO<sub>2</sub> anom.<br>k<sub>w</sub> K<sub>0</sub> contr.<br>(mol m<sup>-2</sup> yr<sup>-1</sup>)"
    i_breaks <- c(-Inf, seq(-0.5, 0.5, 0.1), Inf)
  }
  
  if (i_name == "resid_fgco2_dfco2_kw_sol") {
    i_legend_title <-
      "FCO<sub>2</sub> anom.<br>k<sub>w</sub> K<sub>0</sub> X ΔfCO<sub>2</sub> contr.<br>(mol m<sup>-2</sup> yr<sup>-1</sup>)"
    i_breaks <- c(-Inf, seq(-0.5, 0.5, 0.1), Inf)
  }
...
p_map_mdim_robinson <-
  function(df,
    ...
    if (is.null(dim_col) & is.null(dim_row)) {
      ...
      df_raster <- df %>%
        pivot_wider(names_from = all_of(c(dim_row, dim_col)), values_from = all_of(var)) %>%
        rast(crs = "+proj=longlat")
