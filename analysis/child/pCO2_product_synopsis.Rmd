...
ggplot() +
  ...
  bi_scale_fill(pal = "DkBlue2", dim = dim_set) +
...
```{r hovmoeller_monthly_anomalies, fig.asp=1.2, eval=FALSE}
...
```{r hovmoeller_monthly_anomalies_ensemble_mean, fig.asp=0.4, eval=FALSE}
...
pco2_product_biome_annual_anomaly_ensemble_lm_fgco2_sst <-
  pco2_product_biome_annual_anomaly_ensemble %>%
  filter(name %in% c("fgco2_int", "temperature"), biome == "Global") %>%
  select(year, name, resid) %>%
  pivot_wider(values_from = resid) %>%
...
bind_rows(
  ...
  geom_text(
    ...
    aes(x = year, y = 22.5, label = "Positive"),
    hjust = 0,
    fontface = "bold",
    col = warm_color
  ) +
  geom_text(
    data = . %>%
      filter(year == 1991, name == "temperature"),
    aes(x = year, y = 22.36, label = "Negative"),
    hjust = 0,
    fontface = "bold",
    col = cold_color
  ) +
  geom_text(
    data = . %>%
      filter(year == 1997, name == "temperature"),
    aes(x = year, y = 22.43, label = "annual mean anomaly"),
    hjust = 0,
    fontface = "bold",
    col = "grey40"
  ) +
...
pco2_product_biome_annual_detrended %>%
  filter(biome %in% "Global", name %in% name_core) %>%
...
pco2_product_biome_annual_anomaly %>%
  filter(biome %in% c("Global", key_biomes),
         name %in% name_core) %>%
  mutate(biome = if_else(biome == "Global", "Global non-polar", biome)) %>% 
  select(-c(value, fit)) %>%
  pivot_wider(values_from = resid) %>%
...
pco2_product_biome_annual_fCO2_decomposition %>%
  ...
  scale_color_manual(values = c(warm_color, cold_color, "grey80")) +
  scale_fill_manual(values = c(warm_color, cold_color, "grey80")) +
  labs(y = labels_breaks("sfco2")$i_legend_title) +
