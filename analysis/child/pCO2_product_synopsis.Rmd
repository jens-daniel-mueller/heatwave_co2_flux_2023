...
```{r anomaly_year_annual_mean_maps}
...
```{r anomaly_annual_mean_maps_ensemble_mean, fig.asp=0.5}
...
```{r anomaly_annual_mean_maps_ensemble_mean_offset, fig.asp=0.5}
...
```{r fCO2_decomposition_monthly_maps, fig.asp=1.5}
...
pco2_product_coarse_monthly_fCO2_decomposition %>%
  ...
  map(
    ~ map +
      geom_tile(data = .x,
                aes(lon, lat, fill = resid)) +
      labs(title = paste({{year_anom}}, "thermal vs non-thermal fCO2 anomaly"),
           subtitle = .x$product) +
      scale_fill_continuous_divergingx(
        palette = "RdBu",
        rev = TRUE,
...
pco2_product_coarse_monthly_fCO2_decomposition %>%
  ...
  map(
    ~ map +
      ...
      geom_point(data = .x %>% filter(abs(resid_mean) < resid_sd),
                 aes(lon, lat, shape = "Ensemble mean\n< StDev")) +
      labs(title = paste({{year_anom}},"thermal vs non-thermal anomaly"),
           subtitle = .x$product) +
      scale_fill_continuous_divergingx(
        palette = "RdBu",
        rev = TRUE,
...
```{r fCO2_decomposition_annual_maps, fig.asp=1}
...
map +
  ...
  labs(title = paste({{year_anom}}, "annual mean fCO2 anomaly decomposition")) +
  theme(legend.title = element_markdown(),
        legend.position = "bottom") +
...
pco2_product_coarse_annual_fCO2_decomposition %>%
  ...
  map(
    ~ map +
      ...
      geom_point(data = .x %>% filter(abs(resid_mean) < resid_sd),
                 aes(lon, lat, shape = "Ensemble mean\n< StDev"))+
      labs(title = paste({{year_anom}}, "fCO2 anomaly decomposition"),
           subtitle = .x$product) +
      scale_fill_continuous_divergingx(
        palette = "RdBu",
        rev = TRUE,
...
pco2_product_coarse_monthly_flux_attribution %>%
  ...
  map(
    ~ map +
      geom_tile(data = .x,
                aes(lon, lat, fill = resid)) +
      labs(title = paste({{year_anom}}, "monthly flux anomaly decomposition"),
           subtitle = .x$product) +
      scale_fill_continuous_divergingx(
        palette = "RdBu",
        rev = TRUE,
...
pco2_product_coarse_monthly_flux_attribution %>%
  ...
  map(
    ~ map +
      ...
      geom_point(data = .x %>% filter(abs(resid_mean) < resid_sd),
                 aes(lon, lat, shape = "Ensemble mean\n< StDev"))+
      labs(title = paste({{year_anom}}, "flux anomaly decomposition"),
           subtitle = .x$product) +
      scale_fill_continuous_divergingx(
        palette = "RdBu",
        rev = TRUE,
...
```{r flux_attribution_annual_maps, fig.asp=1}
...
map +
  ...
  labs(title = paste({{year_anom}}, "annual mean flux anomaly decomposition")) +
  theme(legend.title = element_markdown(),
        legend.position = "bottom") +
  facet_grid(product ~ name,
             labeller = labeller(name = x_axis_labels)) +
...
pco2_product_annual_regression %>%
  filter(name %in% c("fgco2", "temperature"),
         biome == "Global") %>%
  ggplot(aes(year, value, col = reorder(product, desc(product)))) +
...
bind_rows(
  ...
  geom_path(
    ...
    aes(year + month / 12, value), col = "grey90"
  ) +
  # geom_step(
  #   data = pco2_product_monthly_regression %>%
  #     filter(
...
bind_rows(
  ...
  scale_fill_brewer(palette = "Set1",
                    direction = -1,
                     labels = c("positive", "negative"),
                    name = "Annual mean anomalies") +
  geom_step(aes(year, resid, col = "Observed")) +
  coord_cartesian(expand = 0) +
  facet_grid(
    name ~ .,
    scales = "free_y",
...
```{r anomalies_correlation_absolute_annual_biome_super, fig.asp=0.8}
