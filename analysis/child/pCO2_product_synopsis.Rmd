...
for(i_file_type in file_types) {
  ...
  if(i_file_type == "map_annual_anomaly"){
    files <- files[!str_detect(files, "slope")]
  }
  ...
  pco2_product <-
    ...
    mutate(
      product = str_extract(
        product,
        "OceanSODA|SOM_FFN|CMEMS|NRT_fco2residual|ETHZ_CESM|FESOM-REcoM"
      )
    )
...
files <- list.files(here::here("data/"),
                    pattern = paste0({{year_anom}},"_map_annual_anomaly.csv"),
                    full.names = TRUE)
...
files <- list.files(here::here("data/"),
                    pattern = paste0({{year_anom}},"_map_monthly_anomaly.csv"),
                    full.names = TRUE)
...
```{r read_hovmoeller_monthly_anomaly}

files <- list.files(here::here("data/"),
                    pattern = paste0({{year_anom}},"_hovmoeller_monthly_anomaly.csv"),
                    full.names = TRUE)
...
```{r read_biome_annual_anomaly}

files <- list.files(here::here("data/"),
                    pattern = paste0({{year_anom}},"_biome_annual_anomaly.csv"),
                    full.names = TRUE)
...
```{r read_biome_monthly_anomaly}

files <- list.files(here::here("data/"),
                    pattern = paste0({{year_anom}},"_biome_monthly_anomaly.csv"),
                    full.names = TRUE)
...
pco2_product_biome_annual_anomaly <- pco2_product_biome_annual_anomaly %>%
  mutate(
    name = factor(name, levels = name_core),
    product = factor(product, levels = all_product_list)
  ) %>%
...
rm(pco2_product_map_annual_anomaly_ensemble_gobm)
gc()
...
rm(pco2_product_map_annual_anomaly_ensemble,
   pco2_product_map_annual_anomaly_ensemble_baseline)
...
pco2_product_map_annual_anomaly_ensemble_correlation <-
  pco2_product_map_annual_anomaly %>%
  filter(product %in% pco2_product_list,
         name %in% c("fgco2", "temperature")) %>%
  fgroup_by(name, lon, lat, year) %>%
  fsummarise(
    resid_mean = fmean(resid),
    n = fnobs(resid)
  ) %>%
...
pco2_product_map_annual_anomaly_ensemble_correlation_lm <-
pco2_product_map_annual_anomaly_ensemble_correlation %>%
  filter(year != {{year_anom}}) %>% 
  pivot_wider(values_from = resid_mean) %>%
...
map +
  geom_tile(data = pco2_product_map_annual_anomaly_ensemble_correlation_lm, 
            aes(lon, lat, fill = slope)) +
  scale_fill_gradientn(
    colours = cmocean("curl")(100),
    rescaler = ~ scales::rescale_mid(.x, mid = 0),
    limits = c(
      quantile(pco2_product_map_annual_anomaly_ensemble_correlation_lm$slope,.01),
      quantile(pco2_product_map_annual_anomaly_ensemble_correlation_lm$slope, .99)),
    oob = squish
  ) +
...
# pco2_product_map_monthly_anomaly_ensemble_correlation_corr <-
#   pco2_product_map_monthly_anomaly_ensemble_correlation %>%
#   pivot_wider(values_from = resid_mean) %>%
#   select(lon, lat, fgco2, temperature) %>%
#   drop_na() %>%
#   nest(data = -c(lon, lat)) %>%
#   mutate(fit = map(data, ~ cor(x = .x$fgco2, y = .x$temperature)))
#   
# pco2_product_map_monthly_anomaly_ensemble_correlation_corr <-
#   pco2_product_map_monthly_anomaly_ensemble_correlation_corr %>%
#   unnest(fit) %>%
#   select(lon, lat, correlation = fit)
# 
# map +
#   geom_tile(data = pco2_product_map_monthly_anomaly_ensemble_correlation_corr, aes(lon, lat, fill = correlation)) +
#   scale_fill_gradientn(
#     colours = cmocean("curl")(100),
#     rescaler = ~ scales::rescale_mid(.x, mid = 0),
#     limits = c(
#       quantile(
#         pco2_product_map_monthly_anomaly_ensemble_correlation_corr$correlation,
#         .01
#       ),
#       quantile(
#         pco2_product_map_monthly_anomaly_ensemble_correlation_corr$correlation,
...
# 
# map +
#   geom_tile(data = pco2_product_map_monthly_anomaly_ensemble_correlation_corr, aes(lon, lat, fill = abs(correlation)))
...
full_join(region_biomes,
          pco2_product_biome_annual_anomaly) %>%
  mutate(biome = if_else(biome == "SP-STSS", "SP-STPS", biome)) %>% 
  mutate(region = case_when(biome == "Global" ~ "Global",
  ...
  mutate(latitude = case_when(
    biome == "Global" ~ "Global",
    biome %in% c("NA-SPSS","NA-STSS","NA-STPS",
                 "NP-SPSS","NP-STSS","NP-STPS") ~ "Northern\nhemisphere",
    biome %in% c("Arabian Sea","Bay of Bengal","Equatorial Indian",
                 "PEQU-W","PEQU-E","AEQU") ~ "Equatorial\nregions",
    biome %in% c("SA-STPS","SP-STPS","Southern Indian") ~ "Southern\nhemisphere",
    TRUE ~ "other"),
...
pco2_product_biome_annual_anomaly_merged %>% 
  filter(product %in% pco2_product_list) %>% 
  group_by(name, biome, region, latitude, biome_class) %>%
  summarise(resid_sd = sd(resid),
            resid = mean(resid))
...
```{r {{year_anom}}_anomaly_profiles_monthly_CESM_hovmoeller, fig.asp=1}
...
ggsave(plot = wrap_plots(plot_list,
       ...
       height = 8,
       dpi = 600,
       filename = "../output/CESM_2023_anomaly_profiles_hovmoeller.jpg")

plot_list <- pco2_product_profiles_monthly %>%
  filter(year == {{year_anom}},
         biome %in% key_biomes,
         product == "ETHZ_CESM",
         name %in% c("sdissic", "thetao")) %>%
...
ggsave(plot = wrap_plots(plot_list,
       ...
       height = 8,
       dpi = 600,
       filename = "../output/CESM_2023_anomaly_profiles_hovmoeller_evolution.jpg")
